...
test('verify that user is not able to checkout without valid information ', async ({ page }) => {
  const loginPage = new Login(page);
  const productPage = new Product(page);
  const yourCartPage = new YourCart(page);
  const checkouPage = new Checkout(page);
  await loginPage.login(CREDENTIALS.SAUCEDEMOUSER, CREDENTIALS.SAUCEDEMOPASSWORD);
  ...
  await expect(checkouPage.errorMessage).toHaveText("Error: First Name is required")
  await checkouPage.enterFirstName(CHECKOUT.FIRSTNAME)
  await checkouPage.clickContinueButton()
  await expect(checkouPage.errorMessage).toHaveText("Error: Last Name is required")
  await checkouPage.enterLastName(CHECKOUT.LASTNAME)
  await checkouPage.clickContinueButton()
  await expect(checkouPage.errorMessage).toHaveText("Error: Postal Code is required")  
});

test('verify that locked out user is not able to login', async ({ page }) => {
  const loginPage = new Login(page);
  await loginPage.login(CREDENTIALS.SAUCEDEMOLOCKEDUSER, CREDENTIALS.SAUCEDEMOPASSWORD);
...
test('verify that invalid user is not able to login', async ({ page }) => {
  const loginPage = new Login(page);
  await loginPage.login(CREDENTIALS.SAUCEDEMOINVALIDUSER, CREDENTIALS.SAUCEDEMOPASSWORD);
