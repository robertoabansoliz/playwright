...
test('verify that user is able to buy a product', async ({ page }) => {
  ...
  await checkouPage.checkout(CHECKOUT.FIRSTNAME, CHECKOUT.LASTNAME, CHECKOUT.ZIPCODE)
  await expect(checkoutOverviewPage.titlePage).toHaveText("Checkout: Overview")
  await expect(checkoutOverviewPage.productTitle).toHaveText("Sauce Labs Backpack")
...
test('verify that user is able to remove to select other product ', async ({ page }) => {
  const loginPage = new Login(page);
  const productPage = new Product(page);
  const yourCartPage = new YourCart(page);
  const checkouPage = new Checkout(page);
  ...
  await checkouPage.checkout(CHECKOUT.FIRSTNAME, CHECKOUT.LASTNAME, CHECKOUT.ZIPCODE)
  await expect(checkoutOverviewPage.titlePage).toHaveText("Checkout: Overview")
  await expect(checkoutOverviewPage.productTitle).toHaveText("Sauce Labs Bike Light")
  await checkoutOverviewPage.clickFinishButton()
  await expect(checkoutCompletePage.successMessage).toHaveText("Thank you for your order!")
  await checkoutCompletePage.clickBackHomeButton()
  await expect(productPage.titlePage).toHaveText("Products");
